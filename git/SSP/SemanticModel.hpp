...
public:
		...
		balance(balance)
	{
		...
		while(!fin.eof()){
			...
			string buf;
			fin>>buf;
			if(data_model.entity_name_to_id.count(buf)==0)
				continue;
			int tempId=data_model.entity_name_to_id.at(buf);
			cout<<tempId<<endl;
			vec& v=v_semantics[tempId];
			v.resize(50);
...
#pragma omp parallel for
		for (auto idoc = documents.begin(); idoc != documents.end(); ++idoc)
		{
			...
			v_semantics[idoc - documents.begin()]=v_doc;
		}
	}
