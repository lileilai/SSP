...
#pragma ivdep
	...
	{
		/*string p_entity2id="./data/entity_word/entity2id.txt";
		fstream fin1(p_entity2id.c_str());
		while(!fin1.eof()){
			...
			fin1>>entityName;
			fin1>>id;
			if (entity_name_to_id.find(entityName) == entity_name_to_id.end())
			{
				entity_name_to_id.insert(make_pair(entityName, entity_name_to_id.size()));
				entity_id_to_name.push_back(entityName);
			}
		}
		fin1.close();
		string p_relation2id="./data/entity_word/relation2id.txt";
		fstream fin2(p_relation2id.c_str());
		while(!fin2.eof()){
			...
			fin2>>relationName;
			fin2>>id;
			if (relation_name_to_id.find(relationName) == relation_name_to_id.end())
			{
				relation_name_to_id.insert(make_pair(relationName, relation_name_to_id.size()));
				relation_id_to_name.push_back(relationName);
			}
		}
		fin2.close();
		fstream fin3(filename.c_str());
		while(!fin3.eof())
		{
			string head, tail, relation;
			fin3>>head>>tail>>relation;
