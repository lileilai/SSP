...
public:
	...
	void load_semantics(){
		...
		string semantic_file="./semantic_v1.txt";
		ifstream fin(semantic_file.c_str(), ios::binary);
		...
		while(!fin.eof()){
			...
			vec v(ve);
			v_semantics.push_back(v);
		}
		cout<<v_semantics.size()<<endl;
		cout<<count_entity()<<" "<<count_relation()<<endl;
		fin.close();
	}
	SemanticModel(
		const Dataset& dataset,
...
#pragma omp parallel for
	...
	virtual void train_batch()
	{
		...
		/*for(int k=0;k<batchSize;k++){
			...
			vec semantic=semantic_composition(triplet);
			vec semantic_f = semantic_composition(triplet_f);
			vec error=head + relation-as_scalar(semantic.t()*relation)*semantic - tail;

			head -= alpha * sign(error);
			tail += alpha * sign(error);
			relation -=alpha * sign(error);

			double projection=as_scalar(semantic.t()*relation);
			head_sem += alpha*projection*(sign(relation - projection*semantic)-
					as_scalar(semantic.t()*sign(relation - projection*semantic)) * sign(head_sem + tail_sem));
			tail_sem += alpha*projection*(sign(relation - projection*semantic)-
								as_scalar(semantic.t()*sign(relation - projection*semantic)) * sign(head_sem + tail_sem));
			vec error_f=head_f + relation_f-as_scalar(semantic_f.t()*relation_f)*semantic_f - tail_f;

			head_f += alpha * sign(error_f) ;
			tail_f -= alpha * sign(error_f);
			relation_f += alpha * sign(error_f);

			double projection_f=as_scalar(semantic_f.t()*relation_f);
			head_sem_f -=alpha*projection_f*(sign(relation_f - projection_f*semantic_f)-
								as_scalar(semantic_f.t()*sign(relation_f - projection_f*semantic_f)) * sign(head_sem_f + tail_sem_f));
			tail_sem_f -=alpha*projection_f*(sign(relation_f - projection_f*semantic_f)-
								as_scalar(semantic_f.t()*sign(relation_f - projection_f*semantic_f)) * sign(head_sem_f + tail_sem_f));
			if (norm_L2(head) > 1.0)
				head = normalise(head);
